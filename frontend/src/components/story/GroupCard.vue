<template>
  <div>
    <div class="group_list_img mx-5 my-5" :style="background_image" >
...
export default {
  ...
  data: function () {
    return {
      ...
      //  backgroundImage: `url(${require('@/assets/story/group_sample_diet.jpg')})`
        ...
        backgroundImage: `url(${SERVER_URL}/club/download/${this.result})`
  ...
  created(){
    axios.get(`${SERVER_URL}/club/${this.group.clubId}`)
    .then((res)=>{
      ...
      this.result = "/uploads/club/"+this.clubdto.data.dto.userId+"/"+this.urls.substr(14+this.clubdto.data.dto.userId.length,(this.clubdto.data.fileUrl[0].length-1) ) 
    
          axios.get(`${SERVER_URL}/club/download${this.result}`
